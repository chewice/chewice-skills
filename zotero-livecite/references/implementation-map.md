...
Use this map after the Skill has triggered and the task requires source inspection or modification. The `zotero_mcp` package lives outside this Skill tree; locate it with `python -c "import zotero_mcp"`. Historical case-specific rebuild scripts are reference-only and must not be imported.

This Skill only converts explicit DOI/PMID markers. EndNote dynamic fields are out of scope.

## Package boundary

Primary Python package (in the importable `zotero_mcp` distribution, typically `src/zotero_mcp/word_citations/` in that repository):
...
| UI evidence | Skill `scripts/validate_word_zotero_ui.ps1` | Cancelled disposable Word/Zotero dialog evidence for citation and bibliography modes. |

This Skill's live wrappers are `scripts/refresh_word_zotero.ps1` and `scripts/validate_word_zotero_ui.ps1`. The package repository may keep copies under `scripts/word_citations/`; do not mix those paths with this Skill root.

## PowerShell boundary

The Skill-local `scripts/refresh_word_zotero.ps1` is the live Word wrapper for this workflow. Static inspection must verify:
...
Do not widen `docx_scan.py` or `placement.py` with EndNote heuristics.
