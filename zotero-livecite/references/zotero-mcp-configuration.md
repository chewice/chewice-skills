...
The live parts of this Skill require **hybrid mode** in zotero-mcp and a working
Zotero MCP connector. Use MCP tools for search, collections, and item
visibility as described in `references/mcp-library.md`. Local reads use the
running Zotero desktop (`http://127.0.0.1:23119`); web-API writes need a
personal API key. Without hybrid mode, every write tool fails with:
...
```json
{
  "client_env": {
    ...
    "ZOTERO_LIBRARY_ID": "<your Zotero user ID, e.g. 21222918>",
