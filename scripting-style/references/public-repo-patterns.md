...
仅在需要源码佐证、比较写法或迭代 Skill 时读取。检查日期：2026-09-18。以下三个项目与 Nature、Science、Cell 正刊论文的对应关系已核对；只读检查选定版本的 source cells，没有执行论文分析，也不使用 stored outputs 证明结论。
...
- 保留既有 [本地范例索引](../examples/example-index.yaml) 的 `<SOURCE_ROOT>` 和同类型路由。本页提供固定公开链接，未提供本地来源根时不搜索用户机器，也不强制下载来源。
