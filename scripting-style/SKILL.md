...
- 在会影响下一步判断的位置观察结果：结构摘要、对象打印、候选表或诊断图都可以是分析本身。读入后先查看，再进入下一步变换；具体 `fn` / `[1:5, 1:5]` 语法留在各文件类型指南。
...
- 读入后是否先查看再变换？矩阵是否用了合适的 `dim()` / `[1:5, 1:5]`，而不是每种对象都堆三种打印？
- 后续还要读的对象是否用了能对上数据的名字，而不是 `tab` / `input` / `c` / `mat` 这类占位名？
- 是否保留了真实探索证据，而没有只留下一个看似确定的最终调用？
- 是否出现没有当前契约支持的 CLI、config、runner、`main()`、`run_analysis()` 或状态管理？
- helper 是否在代表性案例被理解后才抽取，并且没有隐藏科学决定？整段分析是否仍写在顶层，而不是被 named function 包起来？
- 是否减少了会话闸门式断言，同时没有禁用函数或 `stopifnot`？硬断言是否只保护可能静默污染结论的契约？
