...
## Skill tracks versus this package

This map describes the generic `zotero_mcp.word_citations` package, which currently executes **Track A** (explicit DOI/PMID markers → Zotero fields). **Track B** (EndNote dynamic fields → Zotero fields) follows the same protected-source, digest-bound, human-gated contracts in the Skill, but must not import case-specific historical rebuild scripts. When Track B gains package support, add modules here instead of widening `docx_scan.py` or `placement.py` with one-off EndNote heuristics.
