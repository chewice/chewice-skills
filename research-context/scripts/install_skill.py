#!/usr/bin/env python3
"""Install the shared workspace and create discovery links for the Skill."""
...
def parse_args(arguments: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--workspace",
        type=Path,
        default=codex_home / "skill-workspaces/research-context",
    )
    parser.add_argument(
        "--codex-skills-dir",
        type=Path,
...
def validate_workspace(root: Path) -> None:
    required = [
        root / "pixi.toml",
        root / "pixi.lock",
        root / "research-context/SKILL.md",
        root / "research-context/agents/openai.yaml",
    ]
    ...
    nested = [
        ...
        for path in (root / "research-context" / relative,)
        if path.exists()
    ]
    if nested:
